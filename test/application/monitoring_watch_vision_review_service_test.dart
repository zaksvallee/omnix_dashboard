...
void main() {
  group('MonitoringWatchVisionReviewService', () {
    test(
      'openai review fetches digest-auth snapshot and parses json output',
      () async {
        var snapshotGets = 0;
    ...
    test(
      'metadata-only review infers person truth from face matches when object labels are absent',
      () {
        ...
        expect(review.usedFallback, isTrue);
        expect(review.primaryObjectLabel, 'person');
        expect(review.indicatesPerson, isTrue);
        expect(review.indicatesVehicle, isFalse);
      },
    );

    test(
      'openai review falls back to semantic person truth when primary object is omitted',
      () async {
        final client = MockClient((request) async {
          if (request.url.host == '192.168.8.105') {
            return http.Response.bytes(
              utf8.encode('face-image'),
              200,
              headers: {'content-type': 'image/jpeg'},
            );
          }
          return http.Response(
            jsonEncode({
              'output_text': jsonEncode({
                ...
                'risk_delta': 0,
                'tags': <String>[],
                'summary': 'Face visible near the lobby.',
              }),
            }),
            200,
          );
    ...
    test(
      'openai review includes related sequence frames in model request',
      () async {
        late Map<String, dynamic> aiBody;
        final client = MockClient((request) async {
          if (request.url.host == '192.168.8.105') {
