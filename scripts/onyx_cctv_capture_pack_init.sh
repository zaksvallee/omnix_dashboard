...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/onyx_cctv_capture_pack_init.sh [--out-dir <path>] [--site-id <site_id>] [--edge-url <url>] [--camera-id <camera_id>] [--zone <zone>] [--event-id <event_id>]

Purpose:
  Initialize a CCTV pilot capture pack with the exact files expected by the
  CCTV validation scripts and field signoff flow.
USAGE
}
...
- \`bridges.txt\`
  Capture the ONYX \`/bridges\` response after the pilot event.
- \`pollops.txt\`
...
./scripts/onyx_cctv_field_validation.sh \\
  --edge-url ${EDGE_URL:-https://<edge-host>} \\
  --event-id ${EVENT_ID:-<frigate_event_id>} \\
...
echo "PASS: CCTV capture pack initialized: $OUT_DIR"
echo "Files:"
