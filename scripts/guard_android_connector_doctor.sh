...
if [[ "$fallback_count" -gt 0 ]]; then
  ...
  if [[ "$PROVIDER_FAMILY" == "hikvision" ]]; then
    fail "Direct SDK connector missing for $PROVIDER_ID. Vendor SDK classes were not found or fallback activated. First match: $first_match. Ensure Hikvision SDK artifacts are linked and connector class is configured via ONYX_HIKVISION_SDK_CONNECTOR_CLASS if needed."
  fi
  fail "Direct SDK connector missing for $PROVIDER_ID. Vendor SDK classes were not found or fallback activated. First match: $first_match. Ensure FSK SDK artifacts are linked and connector class is configured via ONYX_FSK_SDK_CONNECTOR_CLASS if needed."
fi
