...
def validation_report_consistency_issues(report):
    ...
    parity_data = load_json(parity_report_path)
    parity_integrity_data = load_json(parity_integrity_certificate_json)
    if parity_integrity_data is not None:
        ...
        if not referenced_report_json:
            ...
            referenced_report = load_json(referenced_report_json)
