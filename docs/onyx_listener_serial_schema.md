...
The same staged-path rule now also covers `release_gate.json.validation_report_json`,
which must resolve to the sibling `validation_report.json` in the same artifact
directory as the release gate itself.
