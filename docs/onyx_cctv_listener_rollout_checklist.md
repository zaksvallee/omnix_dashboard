...
- Schema scaffold refs:
  ...
  - Listener validation-trend artifacts now also fail directly on hollow validation bundles, so missing staged evidence in current or previous field-validation runs is surfaced as a trend-layer regression instead of only by later readiness checks.
